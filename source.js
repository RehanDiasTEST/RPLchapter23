...
function getUniqueCoursesForDay(day, currentCourses) {
  const availableCourses = data.mata_kuliah.filter(course => course.hari === day && !currentCourses.includes(course.nama));
...
function generateRandomSchedule() {
  ...
  const saturdayCoursesAvailable = data.mata_kuliah.filter(course => course.hari === 'Sabtu');

  saturdayTimes.forEach(time => {
    const availableCourses = saturdayCoursesAvailable.filter(course => !selectedCourses.includes(course.nama) && course.jam === time);
    if (availableCourses.length > 0) {
      const selectedCourse = availableCourses[Math.floor(Math.random() * availableCourses.length)];
      selectedCourses.push(selectedCourse.nama);
      schedule.push({
...
function generateUniqueSchedules(numSchedules) {
  ...
  return Array.from(uniqueSchedules).map(scheduleStr => JSON.parse(scheduleStr));
}
